...
function getSeeds (callback) 
{
	manager.getSeeds(function (err, ret) 
	{
		...
		if (ret[0]) {
			...
			// console.log("douguo-seeds: " + _queue.queue_seeds.offset + "/" + _queue.queue_seeds.queue.length);
			// console.log(_queue.queue_seeds.queue.slice(-200));
			// _queue.queue_seeds.offset = _queue.queue_seeds.queue.length - 200;
...
function getAnaly (callback) 
{
	manager.getAnaly(function (err, ret) 
	{
		...
		if (ret[0]) {
			...
			console.log("douguo-analy: " + _queue.queue_analy.offset + "/" + _queue.queue_analy.queue.length);
		}
		return callback(null, true);
	});
};
...
function showMessage ()
{
	...
	{
		console.log("~~~~douguo~~~~");
		console.log("种子：" + _queue.queue_seeds.offset + '/' + _queue.queue_seeds.queue.length);
		console.log("分析：" + _queue.queue_analy.offset + '/' + _queue.queue_analy.queue.length);
		console.log("菜谱：" + (_queue.recipes.count - _queue.recipes.array.length)+ '/' +_queue.recipes.count);
	}, 60*1000);
};
