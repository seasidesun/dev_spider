...
function spiderUrl (q_seeds, q_analy)
{
	return function ()
	{
		...
		// console.log("offset/seeds: " + q_seeds.offset + "/" + q_seeds.queue.length);
		...
		for (var i = 0; i < url_seeds.length; i++)
		{
			getHrefFromUrl(url_seeds[i], q_seeds.seeds_key, function (err, hrefs, src_url)
			{
				...
				for (var i = 0; i < hrefs.length; i++)
				{
					...
					if (hrefs[i].indexOf(q_analy.analy_key) != -1) {

						pushArray(hrefs[i], q_analy.queue);
						// var exist = false;
						// // console.log("q_seeds length: " + q_seeds.length);
						// for (var j = q_analy.queue.length - 1; j >= 0; j--)
						// {
						// 	// console.log("analysis exist: " + j);
						// 	if (q_analy.queue[j] === hrefs[i])  {
						// 		exist = true;
						// 		// console.log("one exist:" + hrefs[i]);
						// 		break;
						// 	}
						// };

						// if (!exist) {
						// 	// console.log("add one seeds" + hrefs[i]);
						// 	q_analy.queue.push(hrefs[i]);
						// }


						// console.log("add one analy");
						// q_analy.queue.push(hrefs[i]);
					} else {
						pushArray(hrefs[i], q_seeds.queue);

						// var exist = false;
						// // console.log("q_seeds length: " + q_seeds.length);
						// for (var j = q_seeds.queue.length - 1; j >= 0; j--)
						// {
						// 	// console.log("analysis exist: " + j);
						// 	if (q_seeds.queue[j] === hrefs[i])  {
						// 		exist = true;
						// 		// console.log("one exist:" + hrefs[i]);
						// 		break;
						// 	}
