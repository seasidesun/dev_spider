...
module.exports.spiderUrl =  function (q_seeds, q_analy)
{
	...
	{
		_manager.getHrefFromUrl(url_seeds[i], q_seeds.base_key, function (err, hrefs)
		{
			if (err) {
			...
			if (hrefs) {
				...
				for (var j in hrefs) 
				{
					// console.log(j);
					url = (q_seeds.uri || "") + j;
					if (url.indexOf(q_analy.analy_key) != -1 && url.indexOf("index") === -1) {
						// console.log("get analy: " + url);
						_manager.pushArray(url, q_analy.queue);
					} else if(url.indexOf(q_seeds.seeds_key) != -1 && url.indexOf("jpg") === -1){
						// console.log("get seeds: " + url);
						_manager.pushArray(url, q_seeds.queue);
					}
				}
				url = null;
				hrefs = null;
